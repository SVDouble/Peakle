...
from peakle.localize.outline_score import OutlineScore, rows_to_mask, score_outlines
...
def gt_npz(tmp_path):
    ...
    p = tmp_path / "gt.npz"
    np.savez_compressed(
        p, gt_skyline=sky.astype(np.float32), dem_skyline=sky.astype(np.float32),
        gt_contours=np.packbits(contours), dem_contours=np.packbits(contours),
        shape=np.array([h, w]),
    )
    return p
