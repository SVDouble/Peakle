...
PAL = (
    np.array(
        [(230, 60, 60), (60, 200, 90), (70, 130, 240), (240, 180, 40), (200, 80, 220), (40, 210, 210), (250, 250, 250)],
        float,
    )
    / 255
)
...
def save_rgb(a, p):
    Image.fromarray((np.clip(a, 0, 1) * 255).astype(np.uint8), "RGB").save(p)


def save_gray(a, p):
    Image.fromarray((np.clip(a, 0, 1) * 255).astype(np.uint8), "L").save(p)
...
def walk(skel, max_turn=0.70):
    ...
    def nb(p):
        y, x = p
        return [(y + a, x + b) for a in (-1, 0, 1) for b in (-1, 0, 1) if (a, b) != (0, 0) and (y + a, x + b) in P]
...
def plen(s):
    return sum(np.hypot(s[i][0] - s[i - 1][0], s[i][1] - s[i - 1][1]) for i in range(1, len(s)))


def straightness(s):
    a, b = s[0], s[-1]
    return np.hypot(a[0] - b[0], a[1] - b[1]) / max(plen(s), 1.0)
...
def _endtan(p, at_end, k=6):
    ...
    k = min(k, len(a) - 1)
    v = (a[-1] - a[-1 - k]) if at_end else (a[0] - a[k])
    n = np.hypot(*v)
    return v / n if n else v
...
def merge_polylines(polys, max_gap=14.0, min_cos=0.80):
    ...
    while changed:
        ...
        if best:
            ...
            for k in sorted((ia, ib), reverse=True):
                polys.pop(k)
            polys.append(A + B)
            changed = True
...
def boundary_f(polys, gt_skel, dg, shape):
    img = Image.fromarray(np.zeros(shape, np.uint8))
    dr = ImageDraw.Draw(img)
    for s in polys:
        if len(s) > 1:
            dr.line([(int(x), int(y)) for y, x in s], fill=1, width=1)
    pred = skeletonize(np.asarray(img) > 0)
    if pred.sum() == 0:
        return {"F@10": 0.0, "prec@10": 0.0, "rec@10": 0.0}
    dp = distance_transform_edt(~pred)
    P = float(np.mean(dg[pred] <= 10))
    R = float(np.mean(dp[gt_skel] <= 10))
...
def process(name):
    ...
    rgb = c["rgb"].astype(float)
    depth = c["depth"].astype(float)
    red = c["red"]
    shape = rgb.shape[:2]  # masked to terrain after the SAM3 mask is built (drops t-shirt etc.)
    ...
    b = 4
    mountain[:, :b] = mountain[:, -b:] = mountain[-b:, :] = False
    ...
    has = sky.any(0)
    last = (shape[0] - 1 - sky[::-1, :].argmax(0)).astype(float)
    last[~has] = np.nan
    ...
    im = Image.fromarray((rgb * 0.6 * 255).astype(np.uint8), "RGB")
    dr = ImageDraw.Draw(im)
    dr.line([(int(x), int(r)) for x, r in enumerate(skyline)], fill=(255, 235, 90), width=3)
    im.save(f"{d}/04_skyline.png")
    # 05 depth
    ...
    for i, m in enumerate(layers):
        lo[m & mountain] = lo[m & mountain] * 0.45 + PAL[i % len(PAL)] * 0.55
    ...
    ov = (rgb * 0.45 * 255).astype(np.uint8)
    im = Image.fromarray(ov, "RGB")
    dr = ImageDraw.Draw(im)
    ...
    base = (rgb * 0.5 * 255).astype(np.uint8)
    base[red] = (255, 55, 55)
    im = Image.fromarray(base, "RGB")
    dr = ImageDraw.Draw(im)
    dr.line([(int(x), int(r)) for x, r in enumerate(skyline)], fill=(255, 235, 90), width=3)
    for s in internal:
        dr.line([(int(x), int(y)) for y, x in s], fill=(60, 235, 255), width=2)
    im.save(f"{d}/09_polylines.png")
    # cache final + pre-filter polylines + evidence maps (for the optimizer and offline analysis)
    np.savez(
        f"{d}/_trace.npz",
        polys=np.array(raw_polys, dtype=object),
        internal=np.array(internal, dtype=object),
        labels=np.array(labels, dtype=object),
        depthn=depthn.astype(np.float32),
        edge=E.astype(np.float32),
        sil=(st(sil) if sil.max() else sil).astype(np.float32),
        mountain=mountain,
        red=red,
        skyline=skyline.astype(np.float32),
        shape=shape,
    )
    ...
    m["raw_F@10"] = m_raw["F@10"]
    m["raw_prec@10"] = m_raw["prec@10"]
    m["raw_rec@10"] = m_raw["rec@10"]
    m["edge_source"] = esrc
    m["n_layers"] = len(layers)
    m["n_internal"] = len(internal)
    m["n_removed"] = len(raw_polys) - len(internal)
    ...
    tiles = [
        ...
        "08_ridge_candidates",
        "07_silhouettes",
        "08b_discard_classified",
        "09_polylines",
    ]
    ims = [Image.open(f"{d}/{t}.png").convert("RGB").resize((360, round(360 * shape[0] / shape[1]))) for t in tiles]
    w, h = ims[0].size
    panel = Image.new("RGB", (w * 3, h * 2), (15, 15, 15))
    for i, t in enumerate(ims):
        panel.paste(t, ((i % 3) * w, (i // 3) * h))
    panel.save(f"{d}/panel.png")
    print(
        f"{name}: layers={len(layers)} internal={len(internal)} F@10={m['F@10']} P/R={m['prec@10']}/{m['rec@10']}",
        flush=True,
    )
    return m
