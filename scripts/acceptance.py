...
if bench_files:
    ...
    check(
        "T7",
        "no wrong solve CONFIRMED",
        len(false_conf) == 0,
        f"{len(false_conf)} false / {len(confirmed)} confirmed" + (f" e.g. {false_conf[0]}" if false_conf else ""),
        "0 false",
    )
...
if gtv2_index.exists() and bench_files:
    ...
    check(
        "T8", "GT v2 coverage of bench", len(have) >= 0.8 * len(bench_names), f"{len(have)}/{len(bench_names)}", ">=80%"
    )
    if have:
...
print("\nNotes:")
print(
    "T2 (forward+search self-consistency): covered by `pytest tests/unit/test_localize.py tests/unit/test_gtrefine.py`."
)
